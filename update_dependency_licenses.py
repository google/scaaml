...
LICENSE_OVERRIDES = {
  "capstone": "BSD",
  "vispy": "BSD",
  "chipwhisperer": "BSD",  # This license is exclusively for the capturing tooling.
}
...
def GetPackageLicenses(package_name):
  ...
  for metadata_file in METADATA_FILES:
    ...
    if package_name in LICENSE_OVERRIDES:
      ...
      match = re.search("^License:\s*(.*)$", metadata, re.MULTILINE | re.IGNORECASE)
      license = re.sub("\sLicences|\sLicense,?|Version\s", "", match.group(1))
      license = re.sub("-", " ", license)
    match = re.search("^Home-page:\s*(.*)$", metadata, re.MULTILINE | re.IGNORECASE)
